...
def get_parsed_content_icon(filtered_boxes, starting_idx, image_source, caption_model_processor, prompt=None, batch_size=32):
   print("\n=== get_parsed_content_icon 함수 시작 ===")
   print(f"filtered_boxes: {type(filtered_boxes)}, 길이: {len(filtered_boxes) if filtered_boxes is not None else 'None'}")
   print(f"starting_idx: {starting_idx}")
   print(f"image_source shape: {image_source.shape if image_source is not None else 'None'}")
   ...
   if starting_idx:
       ...
       print(f"필터링된 박스 수 (starting_idx 있음): {len(non_ocr_boxes)}")
   else:
       non_ocr_boxes = filtered_boxes
       print(f"필터링된 박스 수 (starting_idx 없음): {len(non_ocr_boxes)}")
   ...
   for i, coord in enumerate(non_ocr_boxes):
       ...
       print(f"박스 {i} 좌표: xmin={xmin}, xmax={xmax}, ymin={ymin}, ymax={ymax}")
       
       cropped_image = image_source[ymin:ymax, xmin:xmax, :]
       croped_pil_image.append(to_pil(cropped_image))
       print(f"박스 {i} 크롭 완료")
   
   print(f"\n크롭된 이미지 수: {len(croped_pil_image)}")
   
   model, processor = caption_model_processor["model"], caption_model_processor["processor"]
   print(f"\n모델 타입: {type(model)}")
   print(f"프로세서 타입: {type(processor)}")
   ...
   for i in range(0, len(croped_pil_image), batch_size):
       ...
       print(f"\n배치 {i//batch_size + 1} 처리 중 (크기: {len(batch)})")
   ...
   print(f"\n생성된 총 텍스트 수: {len(generated_texts)}")
...
def predict(model, image, caption, box_threshold, text_threshold):
    ...
    results = processor.post_process_grounded_object_detection(
        outputs, inputs.input_ids, box_threshold=box_threshold, text_threshold=text_threshold, target_sizes=[image.size[::-1]]  # 0.4,  # 0.3,
    )[0]
...
def get_som_labeled_img(
    ...
    print("2. get_som_labeled_img 함수 시작")
    ...
    if use_local_semantics:
        ...
        print("아이콘 예측 전:", xyxy, logits, phrases)
