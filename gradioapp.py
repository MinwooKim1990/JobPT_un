...
with gr.Blocks(title="JobPT", css=custom_css, elem_id="cv-analysis-tool") as demo:
   ...
   with gr.Tab("💾 Upload CV"):
       ...
       with gr.Row():
           output_JD_text = gr.Markdown(
               label="🔎 Job Description Result", 
               show_copy_button=True, 
       ...
       input_file.change(
           fn=update_thumbnail, 
           inputs=[input_file], 
           outputs=[upload_thumbnail]
       )
       
       analyze_button.click(
           fn=lambda: "Processing...",  # 시작할 때 상태 메시지
           outputs=status_text,
           queue=False
       ).then(
           fn=analyze_cv,
           inputs=[input_file],
           outputs=[output_JD_text, output_text, link_text, comopany_name_text]
       ).then(
           fn=lambda: "Complete!",  # 완료 메시지
           outputs=status_text,
           queue=False
       )
   ...
   with gr.Tab("📤 Analyze from URL"):
       ...
       url_textbox.change(
           fn=update_thumbnail, 
           inputs=[url_textbox], 
           outputs=[url_thumbnail]
       )
       
       url_analyze_button.click(
           fn=lambda: "Processing...",
           outputs=url_status_text,
           queue=False
       ).then(
           fn=analyze_from_url,
           inputs=[url_textbox],
           outputs=[output_JD_url, output_text_url, link_url, comopany_name_url]
       ).then(
           fn=lambda: "Complete!",
           outputs=url_status_text,
           queue=False
